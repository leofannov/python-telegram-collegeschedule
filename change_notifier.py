...
class ChangeNotifier:
    ...
    def load_settings(self) -> Dict[str, bool]:
        """Загрузка настроек уведомлений об изменениях"""
    ...
    def enable_notifications(self, chat_id: str):
        ...
        self.settings[str(chat_id)] = True
        self.save_settings()
    ...
    def disable_notifications(self, chat_id: str):
        ...
        self.settings[str(chat_id)] = False
        self.save_settings()
    ...
    def is_notification_enabled(self, chat_id: str) -> bool:
        ...
        return self.settings.get(str(chat_id), False)

    def get_all_enabled_chats(self) -> List[str]:
        """Получение всех чатов с включенными уведомлениями"""
        return [chat_id for chat_id, enabled in self.settings.items() 
                if enabled]
    ...
    def check_changes_after_download(self, bot_token: str):
        ...
        if not changed_groups:
            ...
            print("💾 Обновление кэша для всех групп...")
            for group in available_groups:
        ...
        # ВСЕГДА обновляем кэш после проверки
        print("💾 Обновление кэша для всех групп...")
        for group in available_groups:
            self.detector.smart_detector.force_update_cache(group)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_chats = len(self.settings)
        enabled_chats = len(self.get_all_enabled_chats())
